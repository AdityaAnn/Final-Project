...
public class Palindrome {
    ...
    //In Java all functions and variables are declared within a class thus there is no scope resolution operator. Java also does not allow for variables. in java varables must exist within a class. Unlike C++ where glooble varables exist. 
		...
		boolean isPal = true;
		//This will go throught the string and will compare the first and last characters. Then It will incrementing and decrementing the iterators respectively. 
		//If there is a proint when the program does not find a difference and they are not equal, the program stops running.
...
//Compilation
//Java compiles and runs very differently from most other languages first compiling on a platform
//independent compiler then running on a platform dependent virtual machine called the JVM, this is
//why java is able to be compiled once and run anywhere. First the compiler takes the .java source
//code and translates it into bytecode. Then the bytecode is passed to the JVM where is is interpreted
